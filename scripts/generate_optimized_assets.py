...
def _draw_lifetime_overview(result: Mapping[str, Any], path: Path) -> None:
    ...
    axes[0].set_title(f"Dark lifetime {dark['tau_switch_off']:.1f} s")
    ...
    axes[1].set_title(f"Bright lifetime {bright['tau_bright_effective']:.1f} s")
...
def _metrics_markdown(result: Mapping[str, Any]) -> str:
    dark = result["optimized_dark_bright"]["dark_hold"]["operational_model"]
    bright = result["optimized_dark_bright"]["bright_wait"]["effective_model"]
    dark_ci = _interval(dark["bootstrap"]["intervals"], "tau_switch_off__shared")
    bright_ci = _interval(bright["bootstrap"]["intervals"], "tau_bright_effective")
    rows = []
    ...
    return "\n".join(
        [
            ...
            "",
            f"- Dark lifetime: **{dark['tau_switch_off']:.2f} s** "
            f"(95% complete-shot CI {dark_ci['lower']:.2f}–{dark_ci['upper']:.2f} s).",
            f"- Bright/imaging lifetime: **{bright['tau_bright_effective']:.2f} s** "
