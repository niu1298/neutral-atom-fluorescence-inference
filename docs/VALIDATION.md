...
The README now keeps only the sweep-level background-drift figure. The
paired-readout validation figures remain available here:
...
- [Consecutive frames with fitted site ROIs](../assets/readme/paired_images_roi_overlay.png)
- [Background-corrected count distribution and descriptive fit](../assets/readme/count_distribution_fit.png)
- [Paired-frame count scatter](../assets/readme/paired_frame_scatter.png)
- [Site-level descriptive maps](../assets/readme/site_summary_map.png)

These assets are generated by `scripts/generate_readme_assets.py`; the
underlying numerical validation is recorded below and in the machine-readable
reports.

The paired-readout examples use a deterministic representative-data rule:
choose the shot whose mean frame-0 raw ROI count is nearest the run median
after excluding the documented shot-0 background outlier (shot order 50), then
choose the unflagged site whose mean frame-0 corrected count is nearest the
site median (site 160). This rule is generated in
`reports/readme_metrics.md`; no example is selected for visual cleanliness.
